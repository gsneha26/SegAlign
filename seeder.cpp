...
printer_input seeder_body::operator()(seeder_input input) {
    ...
    for (uint32_t i = start_pos; i < end_pos; i += cfg.wga_chunk_size) {
        ...
        if(seed_offset_vector.size() > 0){
            ...
            std::vector<hsp> anchors = g_SeedAndFilter(seed_offset_vector, false, buffer, cfg.seed_size, cfg.xdrop, cfg.hspthresh, cfg.noentropy, cfg.nounique, cfg.nosort); 
    ...
    for (uint32_t i = q_len - end_pos; i < q_len - start_pos; i += cfg.wga_chunk_size) {
        ...
        if(seed_offset_vector.size() > 0){
            seeder_body::num_seeds += seed_offset_vector.size();
            std::vector<hsp> anchors = g_SeedAndFilter(seed_offset_vector, true, buffer, cfg.seed_size, cfg.xdrop, cfg.hspthresh, cfg.noentropy, cfg.nounique, cfg.nosort); 
            seeder_body::num_seed_hits += anchors[0].score;
            if(anchors.size() > 1){
